...
export function createPocketService({
  ...
  injectHtml,
  /** LAN/Quick 访问令牌认证配置（issue #13）：{ getToken, isProtected }，传给代理 */
  auth,
  ...
  function persistAutoTunnel() {
    if (!autoStatePath) return Promise.resolve();
  ...
  return {
      ...
      tunnelPromise = (async () => {
        try {
          ...
          if (cfg?.mode === 'named') {
            ...
            tunnel = { url: `https://${cfg.hostname}`, mode: 'named', kill: result.kill, onExit: result.onExit };
          } else {
            const result = await startTunnel({ port: proxy.port, home, signal: controller.signal, onPhase });
            // 归一化：startTunnel 契约返回 {url, kill}（字符串也兼容）
            tunnel = typeof result === 'string' ? { url: result, mode: 'quick', kill: () => {} } : { ...result, mode: 'quick' };
          }
    ...
    async status() {
      ...
      return {
        ...
        tunnelActiveMode: tunnel?.mode ?? null,
